<?php
namespace Dan\NoleaksParser\DocType\ContoArancio;
...
class Parser extends AbstractParser
{
    ...
    public function getData()
    {
        ...
        foreach($nodeTransactions as $nodeTransaction) {
            ...
            preg_match('/(?P<day>\d{2})\/(?P<month>\d{2})\/(?P<year>\d{4})/', (string)$nodeTds[2], $matches);
            $t['_disponibility_date'] = $matches['year'].'-'.$matches['month'].'-'.$matches['day'];
            $t['description'] = (string)$nodeTds[3];
            $t['amount'] = + str_replace(',','.',str_replace('.','',trim((string)$nodeTds[4])));
            $t['tags'] = array();
            
            switch($t['description']) {
                case 'Trasferim. su C/Predef':
                case 'Giroconto a Conto Corrente Arancio':
                $t['amount'] = - $t['amount'];
            }
...
//        }
    ...
    public function isValid()
    {
        ...
        if (!preg_match('/Consultazioni:(.*)Conto Arancio(.*)Saldo contabile al/s', $content)) {
            return false;
        }
        ...
        if (strpos($this->getContent(), 'Conto Arancio n')===false) {
            return false;
        }
